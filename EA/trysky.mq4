...
//---   
   ...
   if(totalorders == 0)
   {
      ...
      {
         ticket = OrderSend(NULL,OP_SELLLIMIT,buylots*MathPow(buylottimes,i),ctrPrice+(20+buyinterval*i)*Point,3,0,0,"Sky",0,0,clrGreen);
         if(ticket < 0)
            Alert(Symbol(),"buyStop failed");
      }
      for(int i = 0;i < selltotalorders;i++)
      {
         ticket = OrderSend(NULL,OP_BUYLIMIT,selllots*MathPow(selllottimes,i),ctrPrice-(20+sellinterval*i)*Point,3,0,0,"Sky",0,0,clrRed);
         if(ticket < 0)
   ...
   if(AccountProfit() >= 2)//AccountBalance()*profitpercent/100)
